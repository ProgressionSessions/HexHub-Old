-- FULL CREDITS TO VERIAX!


local JB_ENGINE = {}
JB_ENGINE.Doors = {}
JB_ENGINE.Backups = {}
JB_ENGINE.OtherValues = {}
JB_ENGINE.engineFunctions = {}


JB_ENGINE.OtherValues.Time = require(game:GetService("ReplicatedStorage").Resource.Settings).Time
...
for i, v in pairs(getgc(true)) do
	if type(v) == "table" then
		if rawget(v, "Event") and rawget(v, "Fireworks") then
			JB_ENGINE.em = v.em
			JB_ENGINE.GetVehiclePacket = v.GetVehiclePacket
			JB_ENGINE.Fireworks = v.Fireworks
			JB_ENGINE.Network = v.Event
		elseif rawget(v, "State") and rawget(v, "OpenFun") then
			table.insert(JB_ENGINE.Doors, v)
		elseif rawget(v, "Ragdoll") then
			JB_ENGINE.Backups.Ragdoll = v.Ragdoll
		end
...
JB_ENGINE.engineFunctions.Teleport = function(position)
